import {
  PATHNAME_SEARCH,
  PATHNAME_DATASETS,
  PATHNAME_DATA_SERVICES,
  PATHNAME_CONCEPTS,
  PATHNAME_CONCEPTS_COMPARE,
  PATHNAME_INFORMATIONMODELS,
  PATHNAME_REPORTS,
  PATHNAME_ABOUT,
  PATHNAME_ABOUT_REGISTRATION,
  PATHNAME_MAIN_PAGE,
  PATHNAME_NEWS_ARTICLE_V2,
  PATHNAME_FANCY_ARTICLE_V2,
  PATHNAME_GUIDANCE,
  PATHNAME_GUIDANCE_METADATA,
  PATHNAME_ORGANIZATIONS,
  PATHNAME_PUBLIC_SERVICES,
  PATHNAME_PUBLIC_SERVICES_AND_EVENTS,
  PATHNAME_EVENTS,
  PATHNAME_SPARQL,
  PATHNAME_PUBLISHING,
  PATHNAME_ABOUT_DATASETS,
  PATHNAME_ABOUT_DATA_SERVICES,
  PATHNAME_ABOUT_CONCEPTS,
  PATHNAME_ABOUT_INFORMATIONMODELS,
  PATHNAME_ABOUT_HARVESTING,
  PATHNAME_CONTACT_PAGE,
  PATHNAME_AI,
  PATHNAME_REQUESTS,
  PATHNAME_SERVICE_MESSAGES,
  PATHNAME_TRANSPORT_GENERAL,
  PATHNAME_TRANSPORT_ROLES,
  PATHNAME_TRANSPORT_COMPLIANCE,
  PATHNAME_TRANSPORT_ITS,
  PATHNAME_TRANSPORT_ADD,
  PATHNAME_TRANSPORT_NEWS,
  PATHNAME_TERMS_OF_USE,
  PATHNAME_ABOUT_CREATING_DESCRIPTIONS_SELF,
  PATHNAME_ABOUT_CREATING_DESCRIPTIONS_REGISTRATION,
  PATHNAME_ABOUT_PUBLISHING_DESCRIPTIONS,
  PATHNAME_ABOUT_TRIGGERING_HARVEST,
  PATHNAME_ABOUT_RESOURCES
} from './constants/constants';

const routes: any = {
  main: [
    PATHNAME_MAIN_PAGE,
    PATHNAME_SEARCH,
    PATHNAME_DATASETS,
    PATHNAME_DATA_SERVICES,
    PATHNAME_CONCEPTS,
    PATHNAME_INFORMATIONMODELS,
    PATHNAME_PUBLIC_SERVICES_AND_EVENTS,
    PATHNAME_ORGANIZATIONS,
    `${PATHNAME_CONCEPTS}${PATHNAME_CONCEPTS_COMPARE}`,
    `${PATHNAME_CONCEPTS}/:conceptId`,
    `${PATHNAME_INFORMATIONMODELS}/:informationModelId`,
    `${PATHNAME_DATA_SERVICES}/:dataServiceId`,
    `${PATHNAME_PUBLIC_SERVICES_AND_EVENTS}/:publicServiceId`,
    `${PATHNAME_PUBLIC_SERVICES}/:publicServiceId`,
    `${PATHNAME_EVENTS}/:eventId`,
    `${PATHNAME_NEWS_ARTICLE_V2}/:id`,
    `${PATHNAME_FANCY_ARTICLE_V2}/:id`,
    PATHNAME_REPORTS,
    PATHNAME_ABOUT,
    PATHNAME_ABOUT_REGISTRATION,
    PATHNAME_ABOUT_CREATING_DESCRIPTIONS_SELF,
    PATHNAME_ABOUT_CREATING_DESCRIPTIONS_REGISTRATION,
    PATHNAME_ABOUT_PUBLISHING_DESCRIPTIONS,
    PATHNAME_ABOUT_TRIGGERING_HARVEST,
    PATHNAME_ABOUT_RESOURCES,
    PATHNAME_GUIDANCE,
    PATHNAME_GUIDANCE_METADATA,
    PATHNAME_SPARQL,
    PATHNAME_ABOUT_DATASETS,
    PATHNAME_ABOUT_DATA_SERVICES,
    PATHNAME_ABOUT_CONCEPTS,
    PATHNAME_ABOUT_INFORMATIONMODELS,
    PATHNAME_ABOUT_HARVESTING,
    PATHNAME_CONTACT_PAGE,
    PATHNAME_ABOUT_REGISTRATION,
    PATHNAME_TERMS_OF_USE,
    PATHNAME_AI,
    PATHNAME_TRANSPORT_GENERAL,
    PATHNAME_TRANSPORT_ROLES,
    PATHNAME_TRANSPORT_ITS,
    PATHNAME_TRANSPORT_NEWS,
    PATHNAME_TRANSPORT_ADD,
    PATHNAME_TRANSPORT_COMPLIANCE,
    PATHNAME_REQUESTS
  ],
  publishing: [
    PATHNAME_PUBLISHING,
    `${PATHNAME_PUBLISHING}${PATHNAME_ABOUT_REGISTRATION}`,
    `${PATHNAME_PUBLISHING}${PATHNAME_ABOUT_HARVESTING}`,
    `${PATHNAME_PUBLISHING}${PATHNAME_TERMS_OF_USE}`,
    `${PATHNAME_PUBLISHING}${PATHNAME_SERVICE_MESSAGES}`,
    `${PATHNAME_PUBLISHING}${PATHNAME_SERVICE_MESSAGES}/:id`
  ],

  organizations: [
    PATHNAME_ORGANIZATIONS,
    `${PATHNAME_ORGANIZATIONS}/:organizationId`,
    `${PATHNAME_ORGANIZATIONS}/:organizationId/datasets`,
    `${PATHNAME_ORGANIZATIONS}/:organizationId/datasets/:datasetId`
  ]
};

export default routes;
